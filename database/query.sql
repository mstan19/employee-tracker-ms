...
SELECT roles.id, department.department_name AS department, roles.salary FROM roles LEFT JOIN department ON roles.department_id = department.id JOIN employees, employees.manager_id AS manager ON employees.manager_id = employees.id;